...
export default defineCachedEventHandler(async (ctx) => {
  ...
  if (collection) {
    if (icons?.length) {
      ...
      console.log(`[Icon] serving ${(icons || []).map(i => collectionName + ':' + i).join(',')} from bundled collection`)
      return data
    }
  }
  ...
  if (options.fallbackToApi) {
    console.log(`[Icon] fetching ${(icons || []).map(i => collectionName + ':' + i).join(',')} from iconify api`)
